...
- **vinext dev (Cloudflare plugin): 66/73 specs pass; the 7 known gaps are
  ...
  custom-loader/`fill` path, the `/_next/image` 403 middleware branch,
  the gallery scrub redirect, cacheable-404 surrogate headers, the `afterFiles`
  type-ahead rewrite, and the purge endpoint pair (possibly env vars not
  reaching the workerd runtime).
